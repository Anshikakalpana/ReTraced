...
export const recoverStuckJobs = async (queueName: string) => {
  ...
  for (const jobId of expiredJobIds) {
    await redis.multi()
      .lRem(queue.processing, 1, jobId)   // remove from processing
      .zRem(queue.processingZset, jobId)  // remove timestamp
      .rPush(queue.ready, jobId)          // requeue
      .exec();

    console.log("Recovered stuck job:", jobId);
  }
};
